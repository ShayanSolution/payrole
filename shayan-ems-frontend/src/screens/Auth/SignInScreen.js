...
class SignInScreen extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    AuthService.signIn(this.state.email, this.state.password)
      .then((res) => {
        ...
        if (res.success) {
          AuthService.setSession(res.payload);
          this.setState({
            redirectToReferrer: true,
            isSubmitting: false,
          });
          this.props.login(res.payload);
        } else {
  ...
  setServerValidationErrors(failures) {
    ...
    if (Object.prototype.hasOwnProperty.call(failures, "username")) {
      validationErrors.email = failures.username;
    }

    if (Object.prototype.hasOwnProperty.call(failures, "password")) {
